...
DATABASE = os.path.join(BASE_DIR, 'database/sigma_leads.db')
CLIENT_DATABASE = os.path.join(BASE_DIR, 'database/clients.db')
CRM_DATABASE = os.path.join(BASE_DIR, '../HWB-CRM/database/crm.db')
...
def sigma_executive():
    ...
    # Aggregated Digital COPQ (Proving Data Fidelity)
    db_defects = conn.execute('SELECT COUNT(*) FROM COPQ WHERE status != "CLOSED"').fetchone()[0]
    conn.close()
    
    log_errors = 0
    error_log_path = os.path.join(BASE_DIR, "conversion_error.log")
    if os.path.exists(error_log_path):
        with open(error_log_path, "r") as f:
            log_errors = len([line for line in f if line.strip()])

    total_system_exceptions = db_defects + log_errors
    ...
    return render_template('HWB-WEB Sigma Executive.html', 
                         ...
                         defects=defects, 
                         kpivs=kpivs,
                         system_errors=total_system_exceptions)
...
def crm_dashboard():
    ...
    try:
        ...
        clients = []
    leads = conn_sigma.execute('SELECT * FROM Leads ORDER BY timestamp DESC LIMIT 1000').fetchall()
...
def convert_to_client(lead_id):
    try:
        ...
        if lead:
            # ROBUST VALIDATION: Ensure company_name is NOT NULL or EMPTY
            company_name = lead['center_name'] if lead['center_name'] and lead['center_name'].strip() else f"Unknown Facility (Lead #{lead_id})"
            email = lead['email'] if lead['email'] else "N/A"
            
            conn_clients = get_db_connection(CLIENT_DATABASE)
            # Push to clients.db using verified data
            conn_clients.execute('INSERT INTO Customers (company_name, email, company_address) VALUES (?, ?, ?)', 
                               (company_name, email, 'Address Pending Audit'))
            conn_clients.commit()
            conn_clients.close()
            
            # Log transition activity
            conn_sigma.execute('INSERT INTO ClientActivities (client_id, activity_type, description) VALUES (?, "Conversion", ?)', 
                             (lead_id, f"Lead converted to client: {company_name}"))
            conn_sigma.commit()
            flash(f"Successfully converted {company_name} to Client.")
        
        conn_sigma.close()
    except Exception as e:
        with open("conversion_error.log", "a") as f:
            import datetime
            f.write(f"[{datetime.datetime.now()}] Error converting lead {lead_id}: {str(e)}\n")
        flash(f"System Error during conversion: {str(e)}")
