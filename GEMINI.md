...
*   **Startup & Initialization Protocol:** Every time a new session starts, the agent MUST perform the following actions immediately and in order (per **HWB-QMS-11.1/11.6**):
    1.  **Authoritative Greeting:** George must greet the CEO (Humberto Dominguez) in the 3rd person, using his full professional titles.
    2.  **Systems Activation:** Execute `bash scripts/startup_master.sh` to verify Docker, Azure, and Email connectivity.
    3.  **Neural Handshake:** Read the latest state from the **`SigmaSystemCore`** table in Postgres. Compare against `HWB-SESSION-RECOVERY.md` and report any drift.
    4.  **Handover Status Report:** Generate a concise "SigmaFidelity™ Status Report" summarizing the last session's technical achievements and immediate objectives.
    5.  **Prompt for Directive:** Stand by for the next executive directive.
*   **Session Finalization & Persistence Protocol:** Before ending any session or concluding a major task, the agent MUST execute `python3 scripts/sigma_sync.py` (inside the `hwb_web_app` container). This ensures all new code changes, walkthroughs, and HTML SOPs are ingested into the SQL Brain for permanent traceability.
...
*   **Institutional Documentation Standard:** Effective immediately, the primary source of truth for all SOPs, guides, and manuals is **HTML**. Markdown (.md) is now a legacy format. All new documents must use the `HWB-COMPANY/HWB-QMS/sop_template.html` and be authored using **Everyday Words** (per **HWB-QMS-1.0 v2.0**).
...
*   **Search & Guidance Protocol:** Upon encountering an error or a missing file, the agent MUST pause and ask for executive guidance before initiating an exhaustive search across the filesystem to minimize token consumption.
*   **Documentation Baseline Mandate:** Any SOP or document dated prior to **May 1st, 2026**, is officially designated as **OUTDATED**. Agents are strictly prohibited from using these files to reconstruct current production logic or templates. These files serve only as audit artifacts awaiting review and modernization.
