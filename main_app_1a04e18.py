    ...
    try:
        with conn.cursor() as cur:
            ...
            if not lead:
                ...
                return redirect(url_for('admin_operations', view='leads'))
            return render_template('crm_edit_lead.html', lead=lead)
    finally:
        if "conn" in locals(): conn.close()
...
'SELECT * FROM "Warchest" ORDER BY name ASC')
    ...
    return render_template('backoffice_operations.html', work_orders=work_orders, chemicals=chemicals, customers=customers, warchest=warchest)
